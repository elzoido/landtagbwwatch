    ...
    for my $month (@months) {
        ...
        for my $id ( keys %$ds ) {
            ...
            unless ( $db_ds_s->rows ) {
                ...
                $dbh->do(
                    'INSERT INTO drucksachen (periode, periode_id, link, datum, titel) VALUES (?, ?, ?, ?, ?)',
                    undef, $periode, $periode_id,
                    $ds->{$id}->{link},
                    $ds->{$id}->{date},
                    $ds->{$id}->{title}
                );
        ...
        for my $id ( keys %$init ) {
            ...
            unless ( $db_init_s->rows ) {
                ...
                $dbh->do(
'INSERT INTO initiativen (periode, periode_id, urheber_partei, link, datum, art, titel, review) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
                    undef,
                    $periode,
                    $periode_id,
                    $init->{$id}->{urheber},
